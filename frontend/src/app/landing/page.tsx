  ...
  const steps = [
    {
      ...
      body: "Host sets USDC amount, participant fields, and checkout expiry for a Solana-native flow.",
    },
    {
      title: "Share link with attendees",
      body: "Attendees submit details and pay via Solana wallet or QR, all tied to a reference.",
    },
    {
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 md:py-16 space-y-8 md:space-y-10">
        <section className="rounded-2xl app-surface border p-6 md:p-10 shadow-sm">
          ...
          <h1 className="mt-3 text-3xl md:text-5xl font-bold leading-tight max-w-4xl">
            Crypto-native event payment infrastructure for organizers who need reliable USDC deposits.
          </h1>
          <p className="mt-4 text-sm md:text-lg app-muted max-w-3xl">
            Payknot is built for Web3 event teams handling cross-border attendees. It gives
            you a clear payment pipeline with verifiable on-chain references and explicit
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {painPoints.map((item) => (
            <article key={item.title} className="rounded-xl app-surface border p-5 shadow-sm">
        ...
        <section id="how-it-works" className="rounded-2xl app-surface border p-6 md:p-8 shadow-sm">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {steps.map((step, idx) => (
              <article key={step.title} className="rounded-xl border p-4">
                <p className="text-xs font-semibold text-indigo-500">Step {idx + 1}</p>
          ...
          <div className="mt-6 rounded-xl border p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <span className="status-badge pending_payment">pending_payment</span>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <article className="rounded-xl app-surface border p-5 shadow-sm">
            ...
            <ul className="mt-3 space-y-2 text-sm app-muted list-disc pl-4">
              <li>Web3/Solana-native events collecting attendee deposits in USDC.</li>
              <li>Cross-border communities that need predictable, low-friction settlement.</li>
              <li>Teams that need operational clarity for participant payment states.</li>
            </ul>
          </article>
          <article className="rounded-xl app-surface border p-5 shadow-sm">
            <h2 className="text-base font-semibold">Where it does not fit</h2>
            <ul className="mt-3 space-y-2 text-sm app-muted list-disc pl-4">
              <li>General fiat checkout for cards, bank transfers, or legacy PSP tooling.</li>
              <li>Multi-chain abstractions where Solana USDC is not your primary rail.</li>
              <li>Marketplace-style split settlements across many payout destinations.</li>
            </ul>
          </article>
        </section>

        <section className="rounded-2xl app-surface border p-6 md:p-8 shadow-sm">
          <h2 className="text-xl md:text-2xl font-semibold">Credibility, not hype</h2>
          <p className="mt-3 text-sm md:text-base app-muted max-w-3xl">
            Payknot is tested in real event workflows with reliability-first deployment,
            explicit post-deploy checks, and payment verification tied to on-chain references.
            The product is designed to reduce ops ambiguity, not add marketing noise.
          </p>
        </section>
          ...
          <div className="mt-4 space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm app-muted">
                No. Payknot is focused on USDC on Solana for crypto-native event deposits.
              </p>
            </div>
            <div>
              <h3 className="font-semibold">Can attendees verify payment status later?</h3>
              <p className="mt-1 text-sm app-muted">
                Yes. Checkout includes transaction status lookup so users can confirm completed payments.
              </p>
            </div>
            <div>
        ...
        <section className="rounded-2xl border border-indigo-300/70 dark:border-indigo-700/50 bg-gradient-to-r from-indigo-50 to-cyan-50 dark:from-indigo-950/35 dark:to-cyan-950/30 p-6 md:p-8 shadow-sm">
          <h2 className="text-xl md:text-2xl font-semibold">Ready to stop payment confusion?</h2>
          <p className="mt-2 text-sm md:text-base app-muted max-w-2xl">
            Start with a single event and run your next deposit collection with verifiable,
